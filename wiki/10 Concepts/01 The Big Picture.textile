...
Rule construction happens by executing the rule construction code of your Java or Scala code. _parboiled_ provides two separate DSLs (one for Java, one for Scala) that try to make rule definition as "comfortable" as possible under the constraints of the respective source language.
