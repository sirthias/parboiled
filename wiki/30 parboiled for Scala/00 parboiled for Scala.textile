...
_parboild for Scala_ provides a concise internal Scala DSL for parser rule building that enables the use of the
_parboiled_ parsing engine from Scala code without having to resort to the "clunky" Java DSL and "heavy-lifting"
parser extension.

_parboiled_ for Scala uses the exact same parsing engine for rule execution as _parboiled for Java_. The only difference
lies in rule construction, which is done by special Scala DSL constructs. _parboiled for Scala_ offers the following
advantages over its Java counterpart:
...
* Faster initial rule building (no expensive parser extension phase)
